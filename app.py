...
def get_image_groups(main_folder):
    ...
    # Ensure 'input' folder is placed first
    subfolders.sort(key=lambda x: (x != "input", x))
    ...
    return image_groups, subfolders
...
def main():
    ...
    if os.path.exists(main_folder):
        image_groups, subfolders = get_image_groups(main_folder)
        ...
        for base_name, images in image_groups.items():
            ...
            for i, (experiment, image_path) in enumerate(sorted(images, key=lambda x: x[0] != "input")):
                with cols[i]:
                    st.text(experiment)
